# -*- coding: utf-8 -*-
u"""_zf100_gatecount.py —— 门日志**完整性**核对：日志里必须出现脚本声明的每一段（§4.53 的保险）

做法：从 `.ps1` 里抠出所有 `Run-Ps1/Run-Py '<段名>'`，再去日志里找
      `==================== <段名> ====================`；缺一段就 FAIL。
⚠ ZF100 相对 ZF99 多一段（`ZF100 verify`）⇒ 声明 55 段 + 门结束 = 日志 56 段。
"""
import io
import os
import re
...
SCRIPT = os.path.join(TOOLS, u"_zf100_gates.ps1")
LOG = os.path.join(TOOLS, u"_zf100_gates.txt")
...
def main():
    ...
    # ⚠ ★本轮补的一条门（§4.53 的兄弟）：**段"在"不等于段"跑通了"**。
    #   ZF100 第一遍门里 `_zf78_falsify.py` 因为一个没定义的常量当场 NameError，
    #   日志里只有一段 Python traceback —— 而上面那些"段在不在"的检查全绿。
    #   ⇒ 从现在起：日志里出现 `Traceback (most recent call last)` 一律算 FAIL。

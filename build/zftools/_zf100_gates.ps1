# ============================================================
#  _zf100_gates.ps1 —— ZF100（两条机器配方 + 电力高炉新锚点 + 燃烧反应室）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf100_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf100_gatecount.py` 核段数。
#  ⚠ ZF100 相对 ZF99 多一段：`ZF100 verify`（脚本声明 55 段 + 门结束 = 日志 56 段）。
# ============================================================
...
# ZF100 本轮：两条机器配方 + 电力高炉新锚点 + 燃烧反应室
Run-Py  'ZF100 verify'     '_zf100_verify.py' $null
# 反证刀：73 刀（含 ZF100 五刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf100_gatecount.py' $null

# -*- coding: utf-8 -*-
u"""_zf101_gatecount.py —— 门日志**完整性**核对（§4.53 的保险）

做法：从 `.ps1` 里抠出所有 `Run-Ps1/Run-Py '<段名>'`，再去日志里找
      `==================== <段名> ====================`；缺一段就 FAIL。
另加一条（ZF100 起）：**日志里出现 Python traceback 也算 FAIL** —— 段"在"不等于段"跑通了"。
⚠ ZF101 相对 ZF100 多一段（`ZF101 verify`）⇒ 声明 57 段 + 门结束 = 日志 58 段。
"""
import io
import os
import re
...
SCRIPT = os.path.join(TOOLS, u"_zf101_gates.ps1")
LOG = os.path.join(TOOLS, u"_zf101_gates.txt")

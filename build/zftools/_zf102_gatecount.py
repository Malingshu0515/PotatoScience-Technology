# -*- coding: utf-8 -*-
u"""_zf102_gatecount.py —— 门日志**完整性**核对（§4.53 的保险）

⚠ 这份脚本第一版是**用 PowerShell 字符串替换**从 `_zf101_gatecount.py` 生成的
  —— 那正是 §4.53 禁止的做法。发现后**整份重写**（就是你现在看到的这一版），
  违规记在 ZF102 那一行的"我自己的 slip"里。

做法：从 `.ps1` 里抠出所有 `Run-Ps1/Run-Py '<段名>'`，再去日志里找
      `==================== <段名> ====================`；缺一段就 FAIL；
      另外日志里出现 Python traceback 也算 FAIL（段"在"不等于段"跑通了"）。
⚠ ZF102 相对 ZF101 多一段（`ZF102 verify`）⇒ 声明 58 段 + 门结束 = 日志 59 段。
"""
import io
import os
import re
...
SCRIPT = os.path.join(TOOLS, u"_zf102_gates.ps1")
LOG = os.path.join(TOOLS, u"_zf102_gates.txt")

# ============================================================
#  _zf102_gates.ps1 —— ZF102（酸性反应室 + 氢/氯两罐 + 盐酸配方）交付前门检查
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf102_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf102_gatecount.py` 核段数。
#  ⚠ ZF102 相对 ZF101 多一段：`ZF102 verify`（脚本声明 58 段 + 门结束 = 日志 59 段）。
# ============================================================

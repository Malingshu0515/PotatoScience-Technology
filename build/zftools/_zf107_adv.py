...
NODES = [
    ...
    dict(id="capacitor", parent="pressing", frame="task", icon="capacitor",
         crit=("any", ["capacitor"]),
         zh=(u"攒出一颗电容", u"铜锭 + 铝板 + 银板 = 电容；电力高炉、合金炉、太阳能板都要它"),
         en=("Build a Capacitor", "Copper Ingot + Aluminum Plate + Silver Plate = Capacitor; the Electric Blast Furnace, Alloy Smelter and Solar Panel all want one"),
         ja=(u"コンデンサを作ろう", u"銅インゴット + アルミニウム板 + 銀板 = コンデンサ。電力高炉・合金精錬炉・ソーラーパネルが欲しがります"),
         ru=("Соберите конденсатор", "Медный слиток + алюминиевая пластина + серебряная пластина = конденсатор; его хотят доменная печь, плавильня сплавов и солнечная панель")),

    # ============ 主脉：高温与合金 ============
    dict(id="blast_furnace", parent="capacitor", frame="goal", icon="electric_blast_furnace",
         crit=("any", ["electric_blast_furnace"]),
         zh=(u"砌一座高炉", u"一栋 3×3×3 的多方块：正面锚点放原版高炉或主控，再用外壳围起来，摆好右键主控自检"),
         en=("Raise a Blast Furnace", "A 3×3×3 multiblock: put a vanilla blast furnace or the controller on the front and wall it in, then right-click the controller for a self-check"),
         ja=(u"高炉を建てよう", u"3×3×3 のマルチブロック：正面のアンカーに溶鉱炉かコントローラーを置き、外殻で囲います。組んだらコントローラーを右クリックで自己診断"),
         ru=("Постройте домну", "Мультиблок 3×3×3: на переднюю опору — доменная печь или контроллер, обнести корпусом, затем ПКМ по контроллеру для самопроверки")),
    ...
    dict(id="electrolyzer", parent="steel", frame="task", icon="electrolyzer",
         ...
         zh=(u"把水拆开", u"电解器：水 + 电 → 氧气 + 氢气；加海盐再电解 → 氯气 + 氢气"),
         en=("Split the Water", "Electrolyzer: water + power -> oxygen + hydrogen; add sea salt and you get chlorine + hydrogen instead"),
         ja=(u"水を分解しよう", u"電解装置：水 + 電力 → 酸素 + 水素。海塩を加えて電解すると → 塩素 + 水素"),
         ru=("Разложите воду", "Электролизёр: вода + энергия → кислород + водород; добавьте морскую соль → хлор + водород")),
    ...
    dict(id="alloy_smelter", parent="capacitor", frame="goal", icon="alloy_smelter",
         ...
         zh=(u"配出一炉合金", u"主控 + 电源端口 + 炉体：三种金属板 + 电容 + 加热装置 + 散热装置 —— 合金线的起点"),
         en=("Stoke the Alloy Line", "Controller + power port + furnace body: three metal plates + a capacitor + a heater + a heat sink - where the alloy line begins"),
         ja=(u"合金を一炉どうぞ", u"コントローラー + 電源ポート + 炉体：3 種の金属板 + コンデンサ + 加熱装置 + 放熱装置 —— 合金ラインの出発点"),
         ru=("Сплавы в одной печи", "Контроллер + порт питания + корпус: три вида пластин + конденсатор + нагреватель + радиатор — начало линии сплавов")),
    ...
    dict(id="distillation", parent="oil", frame="goal", icon="distillation_controller",
         ...
         zh=(u"把原油拆成五份", u"主控 + 操作员：把原油分成柴油、汽油、石脑油、液化石油气和沥青"),
         en=("Five From One", "Controller + Operator: split crude oil into diesel, gasoline, naphtha, LPG and bitumen"),
         ja=(u"原油を五つに分けよう", u"コントローラー + 操作器：原油をディーゼル、ガソリン、ナフサ、液化石油ガス、アスファルトに分けます"),
         ru=("Пять из одного", "Контроллер + оператор: разделяют сырую нефть на дизель, бензин, нафту, СУГ и битум")),
    ...
    dict(id="sulfur", parent="distillation", frame="task", icon="sulfur",
         crit=("any", ["sulfur"]),
         zh=(u"从沥青里掏出硫", u"沥青 + 氢气进加氢脱硫反应仓 → 硫（硫酸的原料）"),
         en=("Sulfur Out of Bitumen", "Bitumen + hydrogen into the Hydrodesulfurization Chamber -> Sulfur (the raw material for sulfuric acid)"),
         ja=(u"瀝青から硫黄を", u"アスファルト + 水素を水素化脱硫反応チャンバーへ → 硫黄（硫酸の原料）"),
         ru=("Сера из битума", "Битум + водород в камеру гидроочистки → сера (сырьё для серной кислоты)")),

...
④ 资源（blockstate / 模型 / 两张贴图） ⑤ 两条配方（机器那条 + **三元锂那条**） ⑥ 四语言 432 键
⑦ 探针报告 + 文档。
"""
import hashlib
import io
...
def main():
    ...
    for name in LANGS:
        ...
        eq(u"%s 键数 432" % name, EXPECT_KEYS, len(now))
        eq(u"%s 除了新加的 9 个键，别的键与改前件逐字相同" % name, [],
    ...
    check(u"公告键数已重定目标到 432", u"(432 keys each)" in read(

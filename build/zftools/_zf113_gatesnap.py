# -*- coding: utf-8 -*-
u"""_zf109_gatesnap.py —— 全门快照（ZF109 轮，**只读**）

用途：给多会话交接用的一张"现在哪些门是绿的、哪些红、红的为什么"的实况表。
口径（与 `_zf107_gatesnap.py` 完全一样，只是名单加上了本轮）：
  · 逐份跑 `build/zftools/_zf*_verify.py` 这类常驻校验（含 repro / guard 的静态检查）；
  · 每份最多跑 240 秒，超时按**红**记（§4.77：卡死不是绿）；
...
# 只跑"常驻校验"这一类（各轮的 verify/repro/guard），不跑一次性脚本
NAMES = ["_zf64_verify.py", "_zf70_verify.py", "_zf71_verify.py", "_zf72_verify.py",
         ...
         "_zf108_verify.py", "_zf109_verify.py", "_zf111_verify.py", "_zf112_verify.py", "_zf113_verify.py"]
...
def main():
    ...
    for n in NAMES:
        ...
        try:
            ...
            out, rc = u"**超时 240 秒**", 99
        line = u""
        for l in out.split(u"\n"):
            if u"通过" in l and u"失败" in l:
                line = l.strip()
        if not line:
            # 有几份老校验用的是「检查项 = N   失败项 = M」的格式（ZF70/ZF71 那两代），
            # 只认"通过 = …失败 = …"会把它们的汇总行整条丢掉（ZF109 修）
            for l in out.split(u"\n"):
    ...
    if missing:
        ...
        for n in missing:
            print(u"  -- %s" % n)
    print(u"\n判词：绿 %d / 红 %d（红的逐条看上面；本轮**未打包**，所以"
          u"读成品 jar 的那几门红是预期内的）" % (len(green), len(red)))
    return 0

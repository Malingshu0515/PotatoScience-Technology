...
def part_e():
    ...
    check(u"E12 四份都是 492 键（本轮 +12：11 个机键 + 接线口那个）",
          all(len(tables[l]) == 492 for l in tables))
...
def part_f():
    print(u"\n===== F 往轮判据 retarget =====")
    for n, what in ((u"_zf100_verify.py", u"EXPECT_KEYS = 492"),
                    (u"_zf101_verify.py", u"EXPECT_KEYS = 492"),
                    (u"_zf102_verify.py", u"EXPECT_KEYS = 492")):
        p = os.path.join(TOOLS, n)
        check(u"F1 %s 的键数跟到 492" % n, os.path.exists(p) and what in read(p))
    p = os.path.join(TOOLS, u"_zf103_verify.py")
    check(u"F2 _zf103_verify.py 的键数与文案都跟到 492",
          os.path.exists(p) and u"len(table) == 492" in read(p) and u"总键数 492" in read(p))
    left = []

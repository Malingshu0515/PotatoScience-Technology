# -*- coding: utf-8 -*-
u"""_zf126_unprobe.py —— 跑完把探针卸干净（ZF126）

四步：① 先抄存档到 `check/Zf126Check.java`；② 摘掉 `PotatoST` 里那一行钩子（连它前面那个空行）；
③ 删 src 里的探针；④ 逐字节证明 `PotatoST.java` 回到改前件原样。

⚠ 本轮 `PotatoST.java` **只**动了那一行钩子（ZF125 的能力登记早就提交了）
   ⇒ 预期状态就是 `zf126_pre` 里那份，不需要"改前件 + 补丁"。

跑法：
    python build\\zftools\\_zf126_unprobe.py
"""
import hashlib
import io
import os
...
SRC = os.path.join(JAVA, "Zf126Check.java")
ARCH = os.path.join(ROOT, r"build\zftools\check\Zf126Check.java")
MAIN = os.path.join(JAVA, "PotatoST.java")
BK = r"C:\PotatoST救援\zf126_pre\src\main\java\com\potatost\mod\PotatoST.java"
HOOK = u"        Zf126Check.register();   // ← 临时探针（ZF126），跑完删\n"
...
def main():
    ...
    elif u"Zf126Check" not in text:
        notes.append(u"② PotatoST 里本来就没有挂载（已经卸过）")
    else:
        fails.append(u"PotatoST 里还有 Zf126Check，但找不到那一行的原样文本 —— 别再瞎删")

    if os.path.exists(SRC):
        os.remove(SRC)
        notes.append(u"③ 删掉探针源文件 Zf126Check.java")

    after = io.open(MAIN, encoding="utf-8", newline=u"").read()
    if u"Zf126Check" in after:
        fails.append(u"PotatoST.java 里还残留 Zf126Check")
    if os.path.exists(SRC):
    ...
    if not os.path.exists(BK):
        ...
        before = open(BK, "rb").read()
        cur = open(MAIN, "rb").read()
        if sha(BK) == sha(MAIN):

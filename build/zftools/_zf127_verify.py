...
七段（判据尽量钉在**用户点名的那些字**上）：
  ...
  E 语言：四语言各 **492** 键、只有那两个新键、其余键与改前件逐字相同、键序不乱；
...
def part_e():
    ...
    check(u"E1 四份各 492 键（本轮 +2）", all(len(t) == 492 for t in tables.values()))
...
def part_f():
    ...
    check(u"F7 公告键数 = 492", u"(492 keys each)" in ann)
...
def part_g():
    ...
    check(u"G4 交接文档的键数活体数字 = 492", u"**492 键 × 4**" in hand or u"492 键 × 4" in hand)

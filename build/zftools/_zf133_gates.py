# -*- coding: utf-8 -*-
"""_zf133_gates.py —— ZF133 的九道门（§11.1，一道都不能跳）
...
⚠ **必须用 scriptblock 方式调 PowerShell 脚本**（这三个 .ps1 自己头部就写了原因）：
  ...
  报的却是"缺少右花括号"这种**指向错误方向的**语法错（我第一版就是这么被误导的：
  以为三个门脚本坏了，其实是调用方式不对）。
  ⇒ 这条要进档案：**"门报语法错"先怀疑调用方式，别急着改门。**

跑法：python build\\zftools\\_zf133_gates.py
"""
import io
import os
import subprocess
...
OUT = os.path.join(T, "_zf133_gates.txt")

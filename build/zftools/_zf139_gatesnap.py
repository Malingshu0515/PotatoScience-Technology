# -*- coding: utf-8 -*-
u"""_zf139_gatesnap.py —— 全门快照（ZF139 轮，**只读**）

口径与 `_zf107/_zf109/_zf111/_zf112/_zf113/_zf119/_zf121/_zf125/_zf128_gatesnap.py` 完全一样。
名单在 ZF128 那份的基础上补上它漏掉的几道（`_zf120_verify.py` / `_zf122_verify.py` /
`_zf133_verify.py` / `_zf134_verify.py` / `_zf135_verify.py`）—— **改前改后跑的是同一份名单**，
所以两次数出来的绿/红可以直接逐条比。
...
跑法：
    python build\\zftools\\_zf139_gatesnap.py --out build\\zftools\\_zf139_gatesnap_before.txt
"""
import io
import os
import subprocess

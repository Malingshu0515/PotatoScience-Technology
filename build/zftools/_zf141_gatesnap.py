# -*- coding: utf-8 -*-
u'''_zf141_gatesnap.py —— 全门快照（ZF141 轮，**只读**）

口径与 `_zf107/_zf109/_zf111/_zf112/_zf113/_zf119/_zf121/_zf125/_zf128/_zf139_gatesnap.py` 一样。
名单在 ZF139 那份的基础上补两道**它漏掉的**：`_zf139_verify.py`（上一轮自己那道门居然不在名单里）
与 `_zf141_verify.py`（本轮）。

  · 逐份跑 `build/zftools/_zf*_verify.py` 这类常驻校验（含 repro / guard / tabaudit 的静态检查）；
  · 每份最多 300 秒，超时按**红**记（§4.77：卡死不是绿）；
  · 只打印「通过/失败」那行 + 前 3 条 FAIL，不打印全文（全文留在各自的输出里）。
⚠ 本脚本**不修改任何文件**；但它跑的某些门会临时改源码（falsify 系列不在名单里，安全）。

跑法：
    python build\\zftools\\_zf141_gatesnap.py --out build\\zftools\\_zf141_gatesnap.txt
'''
import io
import os
import subprocess
...
NAMES = ["_zf64_verify.py", "_zf70_verify.py", "_zf71_verify.py", "_zf72_verify.py",
         ...
         "_zf119_verify.py", "_zf120_verify.py", "_zf121_verify.py", "_zf122_verify.py",
         "_zf123_verify.py", "_zf123_langaudit.py", "_zf124_verify.py", "_zf125_verify.py",
         "_zf126_verify.py", "_zf127_verify.py", "_zf128_verify.py",
         "_zf133_verify.py", "_zf134_verify.py", "_zf135_verify.py",

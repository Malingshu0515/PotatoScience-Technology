# -*- coding: utf-8 -*-
r'''_zf145_unprobe.py —— 摘掉 ZF145 的探针（§10：探针不许跟着提交上车）

四件事：
  ① 把 `src\main\java\com\potatost\mod\Zf145Check.java` **先抄进**
     `build\zftools\check\Zf145Check.java`（留档）**再从 src 删掉**；
  ② ⚠ **自检「归档物与产物自洽」**（§4.145）：归档件的 TAG / 类名 / 报告路径必须与报告文件里
     **实际出现**的 TAG 对得上 —— ZF139 那次就是对不上而没人发现；
  ③ 从 `PotatoST.java` 删掉那一块（注释 + `Zf145Check.register();`，含前导换行）；
  ④ **自证互逆**：`PotatoST.java` 必须与改前件（`zf145_pre`）**逐字节相同**。

跑法：python build\zftools\_zf145_unprobe.py [--keep-src]
'''
import hashlib
import io
import os
...
JAVA = os.path.join(SRC, u"Zf145Check.java")
POT = os.path.join(SRC, u"PotatoST.java")
REPORT = os.path.join(ROOT, r"build\zftools\_zf145_probe_utf8.txt")

BLOCK = (u"\n        // \u26a0\u26a0 临时探针（ZF145）：成就树补线（43 条 + 真触发 + 伤害类型标签），"
         u"跑完由 _zf145_unprobe.py 删掉\n        Zf145Check.register();")

TAG = u"[A145] "
CLASS = u"public final class Zf145Check"
REPORT_NAME = u"_zf145_probe_utf8.txt"
...
def main(argv):
    ...
    if os.path.exists(JAVA):
        ...
        shutil.copy2(JAVA, os.path.join(CHECK, u"Zf145Check.java"))
        if sha1(JAVA) != sha1(os.path.join(CHECK, u"Zf145Check.java")):
            fails.append(u"留档哈希不一致")
        else:
            notes.append(u"  [留档] build/zftools/check/Zf145Check.java（与源逐字节相同）")
        if not keep:
            os.remove(JAVA)
            notes.append(u"  [删除] src/main/java/com/potatost/mod/Zf145Check.java")
    else:
        notes.append(u"  [已做过] 源里没有 Zf145Check.java")

    arc = os.path.join(CHECK, u"Zf145Check.java")
    if not os.path.isfile(arc):
        ...
        if not os.path.isfile(REPORT):
            ...
            notes.append(u"  [报告] [OK] %d 条 / [FAIL] %d 条 / 判词 %s"
                         % (ok, fa, u"全绿" if u"verdict: ALL OK" in rep else u"**不是全绿**"))
            if fa or u"verdict: ALL OK" not in rep:
    ...
    if u"Zf145Check" in text:
        ...
        notes.append(u"  [已做过] PotatoST.java 里没有 Zf145Check")

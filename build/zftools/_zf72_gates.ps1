# ============================================================
#  _zf72_gates.ps1 —— ZF72 交付前门检查（**规划轮：不动 jar**）+ 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕开）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf72_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
# ============================================================
...
# ZF72 本轮：① 从 jar 现抠原版证据（每次重抠，原版/NeoForge 换版本会变）
Run-Py  'ZF72 vanilla evidence' '_zf72_vanilla_evidence.py' $null
#             ② 规划文档 vs 代码 vs 原版证据 vs 冻结状态（57 项）

# ============================================================
#  _zf82_gates.ps1 —— ZF82（0.11 柴油桶/汽油桶 + 容器换流器）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕开）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf82_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 这个文件必须**整份写**（用文件工具），不许用 PowerShell 的字符串替换去改（§4.53：那样会把
#     Run 行并进注释里，门少跑一半而日志照样"全绿"）。跑完用 `_zf82_gatecount.py` 核段数。
# ============================================================
...
# ZF82 本轮：柴油桶/汽油桶（世界流体 + 官方桶）+ 容器换流器（3 秒换桶 + 泵直接抽左槽容器）
Run-Py  'ZF82 verify'      '_zf82_verify.py'  $null
# 反证台：22 刀（含 ZF82 三刀）—— 每刀都要被抓到、逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志里的段数必须等于脚本里声明的段数（§4.53）
Run-Py  'gatecount'        '_zf82_gatecount.py' $null

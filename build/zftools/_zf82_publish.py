# -*- coding: utf-8 -*-
u"""_zf82_publish.py —— ZF82 出成品（作废 ZF81 那版 `257ff4b7…`）+ 填文档占位 + 把 ZF81 条目的"成品"降格
...
VOID = "8ba61f5d957fd0d809f1d0c2b811b739b733e7be"
fails = []
...
def main():
    ...
    print(u"   作废 %s（ZF81）" % VOID[:8])

    patch(DOC, u"__NEWSHA__", new, u"§9 ZF82 条目：成品哈希填实")
    patch(DOC, u"__NEWSIZE__", str(size), u"§9 ZF82 条目：字节数填实")
    patch(DOC, u"__NEWENTRIES__", str(entries), u"§9 ZF82 条目：条目数填实")
    patch(DOC, u"**成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"**当时的成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"§9 ZF81 条目：成品 → 当时的成品")

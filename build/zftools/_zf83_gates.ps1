# ============================================================
#  _zf83_gates.ps1 —— ZF83（0.11 三张板子贴图换新）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕开）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf83_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 这个文件必须**整份写**（用文件工具），不许用 PowerShell 字符串替换去改（§4.53）。
#     跑完用 `_zf83_gatecount.py` 核段数。
# ============================================================
...
# ZF73：① 复现性（除新增那几份，其余配方逐字节没动）② 交付校验
...
# ZF81：电解器能耗 100 → 1000 FE/t
Run-Py  'ZF81 verify'      '_zf81_verify.py'  $null
# ZF82：柴油桶/汽油桶 + 容器换流器（3 秒换桶 + 泵直接抽左槽容器）
Run-Py  'ZF82 verify'      '_zf82_verify.py'  $null
# ZF83 本轮：三张板子贴图换新（user 原图转档 + 三个模型指向自己的贴图）
Run-Py  'ZF83 verify'      '_zf83_verify.py'  $null
# 反证台：23 刀（含 ZF83 那一刀）—— 每刀都要被抓到、逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志里的段数必须等于脚本里声明的段数（§4.53）
Run-Py  'gatecount'        '_zf83_gatecount.py' $null

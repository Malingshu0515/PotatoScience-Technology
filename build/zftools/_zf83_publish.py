# -*- coding: utf-8 -*-
u"""_zf83_publish.py —— ZF83 出成品（作废 ZF82 那版 `8ba61f5d…`）+ 填文档占位
...
SHA = DST + ".sha1"
PUB = os.path.join(ROOT, "build", "zftools", "_zf83_publish.py")
DOC = os.path.join(ROOT, "docs", u"开发档案.md")
VOID = "9e2a9e25882dd0f27ecc2392c63ccea1cfc2b015"
fails = []
...
def main():
    ...
    print(u"   作废 %s（ZF82）" % VOID[:8])

    patch(DOC, u"__NEWSHA__", new, u"§9 ZF83 条目：成品哈希填实")
    patch(DOC, u"__NEWSIZE__", str(size), u"§9 ZF83 条目：字节数填实")
    patch(DOC, u"__NEWENTRIES__", str(entries), u"§9 ZF83 条目：条目数填实")
    patch(DOC, u"**成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"**当时的成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"§9 ZF82 条目：成品 → 当时的成品")

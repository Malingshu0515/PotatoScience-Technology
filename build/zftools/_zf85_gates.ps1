# ============================================================
#  _zf85_gates.ps1 —— ZF85（0.11 清掉 ModFluids 的 25 条 IDE 警告/报错）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕开）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf85_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不许 PowerShell 字符串替换（§4.53）。跑完用 `_zf85_gatecount.py` 核段数。
# ============================================================
...
# ZF85 本轮：弃用的 initializeClient 搬家 + 删死代码（行为零改动）
Run-Py  'ZF85 verify'      '_zf85_verify.py'  $null
# 反证台：25 刀（含 ZF85 两刀）—— 每刀都要被抓到、逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf85_gatecount.py' $null

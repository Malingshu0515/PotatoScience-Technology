# -*- coding: utf-8 -*-
u"""_zf86_publish.py —— ZF86 出成品（作废 ZF85 那版 `cd404f6b…`）+ 填文档占位
...
SHA = DST + ".sha1"
PUB = os.path.join(ROOT, "build", "zftools", "_zf86_publish.py")
DOC = os.path.join(ROOT, "docs", u"开发档案.md")
VOID = "ba7ecc97eff87461704849b35680d632295c8079"
fails = []
...
def main():
    ...
    print(u"   作废 %s（ZF85）" % VOID[:8])

    patch(DOC, u"__NEWSHA__", new, u"§9 ZF86 条目：成品哈希填实")
    patch(DOC, u"__NEWSIZE__", str(size), u"§9 ZF86 条目：字节数填实")
    patch(DOC, u"__NEWENTRIES__", str(entries), u"§9 ZF86 条目：条目数填实")
    patch(DOC, u"**成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"**当时的成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"§9 ZF85 条目：成品 → 当时的成品", expect=0)

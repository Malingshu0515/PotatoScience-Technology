# ============================================================
#  _zf87_gates.ps1 —— ZF87（0.11 油桶贴图 + 四张素材转档）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕开）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf87_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不许 PowerShell 字符串替换（§4.53）。跑完用 `_zf87_gatecount.py` 核段数。
# ============================================================
...
Run-Py  'gatecount'        '_zf87_gatecount.py' $null

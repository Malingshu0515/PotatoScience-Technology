# -*- coding: utf-8 -*-
u"""_zf87_publish.py —— ZF86 出成品（作废 ZF85 那版 `cd404f6b…`）+ 填文档占位
...
SHA = DST + ".sha1"
PUB = os.path.join(ROOT, "build", "zftools", "_zf87_publish.py")
DOC = os.path.join(ROOT, "docs", u"开发档案.md")
VOID = "3659d7ce8937f8f80fbabdd7cb4e5f557f4e05fa"
fails = []
...
def main():
    ...
    print(u"   作废 %s（ZF86）" % VOID[:8])

    patch(DOC, u"__NEWSHA__", new, u"§9 ZF87 条目：成品哈希填实")
    patch(DOC, u"__NEWSIZE__", str(size), u"§9 ZF87 条目：字节数填实")
    patch(DOC, u"__NEWENTRIES__", str(entries), u"§9 ZF87 条目：条目数填实")
    patch(DOC, u"**成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"**当时的成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"§9 ZF86 条目：成品 → 当时的成品", expect=0)

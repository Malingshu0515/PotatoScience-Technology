# ============================================================
#  _zf88_gates.ps1 —— ZF88（0.11 原油/柴油流体贴图）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕开）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf88_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不许 PowerShell 字符串替换（§4.53）。跑完用 `_zf88_gatecount.py` 核段数。
# ============================================================
...
# ZF87/ZF88：油桶贴图 + 四张素材 + 原油/柴油流体贴图（都是「用户放图 ⇒ 转档 ⇒ 指向自己」）
...
Run-Py  'gatecount'        '_zf88_gatecount.py' $null

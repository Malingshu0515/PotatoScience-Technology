# -*- coding: utf-8 -*-
u"""_zf88_publish.py —— ZF86 出成品（作废 ZF85 那版 `cd404f6b…`）+ 填文档占位
...
SHA = DST + ".sha1"
PUB = os.path.join(ROOT, "build", "zftools", "_zf88_publish.py")
DOC = os.path.join(ROOT, "docs", u"开发档案.md")
VOID = "f86c569c9dd5a9bafb6009c849f29e743ee97c41"
fails = []
...
def main():
    ...
    print(u"   作废 %s（ZF87）" % VOID[:8])

    patch(DOC, u"__NEWSHA__", new, u"§9 ZF88 条目：成品哈希填实")
    patch(DOC, u"__NEWSIZE__", str(size), u"§9 ZF88 条目：字节数填实")
    patch(DOC, u"__NEWENTRIES__", str(entries), u"§9 ZF88 条目：条目数填实")
    patch(DOC, u"**成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"**当时的成品**：`release\\PotatoST-0.11.jar` = `%s`" % VOID,
          u"§9 ZF87 条目：成品 → 当时的成品", expect=0)

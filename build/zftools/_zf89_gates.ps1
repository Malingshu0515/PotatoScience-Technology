# ============================================================
#  _zf89_gates.ps1 —— ZF89（0.11 汽油/石脑油流体贴图）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf89_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf89_gatecount.py` 核段数。
#  ⚠ ZF89 相对 ZF88 只多一段：`ZF89 verify`（42 段声明 + 门结束 = 日志 43 段）。
# ============================================================
...
# ZF89 本轮：汽油 / 石脑油流体贴图 + 电力高炉那条「已知待修」记账
Run-Py  'ZF89 verify'      '_zf89_verify.py'  $null
# 反证刀：28 刀（含 ZF89 三刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf89_gatecount.py' $null

# ============================================================
#  _zf90_gates.ps1 —— ZF90（0.11 板子换铁板贴图 + 中途两张桶贴图）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf90_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf90_gatecount.py` 核段数。
#  ⚠ ZF90 相对 ZF89 只多一段：`ZF90 verify`（43 段声明 + 门结束 = 日志 44 段）。
# ============================================================
...
# ZF90 本轮：四个板子模型改指 iron_plate + 通用 plate.png 已删 + 柴油桶/汽油桶拿到自己的图 + 三个工具雷
Run-Py  'ZF90 verify'      '_zf90_verify.py'  $null
# 反证刀：32 刀（含 ZF90 四刀、ZF89 三刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf90_gatecount.py' $null

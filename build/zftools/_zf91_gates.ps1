# ============================================================
#  _zf91_gates.ps1 —— ZF91（0.11 电力高炉换成带真 UV 的模型）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf91_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf91_gatecount.py` 核段数。
#  ⚠ ZF91 相对 ZF90 只多一段：`ZF91 verify`（44 段声明 + 门结束 = 日志 45 段）。
# ============================================================
...
# ZF91 本轮：四份 OBJ 换成用户 .bbmodel 烘出的真 UV 版（贴图一字节未动）
Run-Py  'ZF91 verify'      '_zf91_verify.py'  $null
# 反证刀：35 刀（含 ZF91 三刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf91_gatecount.py' $null

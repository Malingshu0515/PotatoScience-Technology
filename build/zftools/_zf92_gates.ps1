# ============================================================
#  _zf92_gates.ps1 —— ZF92（0.11 电力高炉两根接线柱的贴图对调）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf92_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf92_gatecount.py` 核段数。
#  ⚠ ZF92 相对 ZF91 多两段：`ZF92 verify` 与 `ZF92 obj diff`（脚本声明 46 段 + 门结束 = 日志 47 段）。
# ============================================================
...
# ZF92 本轮：两根接线柱的 UV 归属（顶=素板 / 正面=金框 / 底=盖板）；贴图与顶点一字节未动
Run-Py  'ZF92 verify'      '_zf92_verify.py'  $null
# OBJ 逐行对账：四份各 841 行不变、差异 20 行全是 vt
Run-Py  'ZF92 obj diff'    '_zf92_diff.py'    $null
# 反证刀：38 刀（含 ZF92 三刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf92_gatecount.py' $null

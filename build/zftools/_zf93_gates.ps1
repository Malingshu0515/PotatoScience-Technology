# ============================================================
#  _zf93_gates.ps1 —— ZF93（0.11 第二张音乐唱片《茉莉花（管弦乐）》）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf93_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf93_gatecount.py` 核段数。
#  ⚠ ZF93 相对 ZF92 多一段：`ZF93 verify`（脚本声明 47 段 + 门结束 = 日志 48 段）。
# ============================================================
...
# ZF93 本轮：第二张音乐唱片《茉莉花（管弦乐）》（时长现场量 / 两条算法互核 / 6 处联动）
Run-Py  'ZF93 verify'      '_zf93_verify.py'  $null
# 反证刀：42 刀（含 ZF93 四刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf93_gatecount.py' $null

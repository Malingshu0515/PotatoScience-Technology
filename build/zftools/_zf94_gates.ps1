# ============================================================
#  _zf94_gates.ps1 —— ZF94（0.11 电力高炉两根接线柱的东/西统一）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf94_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf94_gatecount.py` 核段数。
#  ⚠ ZF94 相对 ZF93 多两段：`ZF94 verify` 与 `ZF94 obj diff`（脚本声明 49 段 + 门结束 = 日志 50 段）。
# ============================================================

# ============================================================
#  _zf95_gates.ps1 —— ZF95（0.11 用户口述的 5 条合成配方）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf95_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf95_gatecount.py` 核段数。
#  ⚠ ZF95 相对 ZF94 多一段：`ZF95 verify`（脚本声明 50 段 + 门结束 = 日志 51 段）。
# ============================================================
...
# ZF95 本轮：用户口述的 5 条合成配方（两张唱片 + 合金炉主控 + 分馏塔控制器/操作器）
Run-Py  'ZF95 verify'      '_zf95_verify.py'  $null
# 反证刀：49 刀（含 ZF95 四刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf95_gatecount.py' $null

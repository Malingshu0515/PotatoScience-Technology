# -*- coding: utf-8 -*-
u"""_zf97_gatecount.py —— 门日志**完整性**核对：日志里必须出现脚本声明的每一段（§4.53 的保险）

做法：从 `.ps1` 里抠出所有 `Run-Ps1/Run-Py '<段名>'`，再去日志里找
      `==================== <段名> ====================`；缺一段就 FAIL。

⚠ 本文件第一版是用 PowerShell `-replace` 从 `_zf96_gatecount.py` 复制的 —— 那是 §4.53
   **明令禁止**的写法（"用字符串替换生成脚本"）。发现后立刻用文件工具整份重写，
   内容与那份复制品逐行相同（只多了这段说明）。
"""
import io
import os
import re
...
SCRIPT = os.path.join(TOOLS, u"_zf97_gates.ps1")
LOG = os.path.join(TOOLS, u"_zf97_gates.txt")

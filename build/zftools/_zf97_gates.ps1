# ============================================================
#  _zf97_gates.ps1 —— ZF97（空气分离器 + 氨气组成室 + 氮气/氨气）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf97_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf97_gatecount.py` 核段数。
#  ⚠ ZF97 相对 ZF96 多一段：`ZF97 verify`（脚本声明 52 段 + 门结束 = 日志 53 段）。
# ============================================================
...
# ZF97 本轮：空气分离器 + 氨气组成室 + 两种新气体（氮气/氨气）
Run-Py  'ZF97 verify'      '_zf97_verify.py'  $null
# 反证刀：60 刀（含 ZF97 六刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf97_gatecount.py' $null

# ============================================================
#  _zf98_gates.ps1 —— ZF98（流体泵改成"不存流体、只做传输"）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf98_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf98_gatecount.py` 核段数。
#  ⚠ ZF98 相对 ZF97 多一段：`ZF98 verify`（脚本声明 53 段 + 门结束 = 日志 54 段）。
# ============================================================
...
# ZF98 本轮：流体泵不存流体、只做传输、优先送目标收得下的
Run-Py  'ZF98 verify'      '_zf98_verify.py'  $null
# 反证刀：65 刀（含 ZF98 五刀）—— 每刀都要被抓到。逐刀还原回全绿
Run-Py  'falsify'          '_zf78_falsify.py' $null
# 门自己也要自证：日志段数必须等于脚本声明的段数（§4.53）
Run-Py  'gatecount'        '_zf98_gatecount.py' $null

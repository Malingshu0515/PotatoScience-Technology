# ============================================================
#  _zf99_gates.ps1 —— ZF99（空气分离器工作时的白色烟雾粒子）交付前门检查 + 往轮盘面复核 + 本轮校验
#  用法（本机执行策略 Restricted，用 scriptblock 绕过）：
#    $sb=[scriptblock]::Create([IO.File]::ReadAllText('E:\PotatoST\build\zftools\_zf99_gates.ps1',[Text.Encoding]::UTF8)); & $sb *> 日志
#
#  ⚠ 整份写（用文件工具），不让 PowerShell 字符串替换（§4.53）。跑完用 `_zf99_gatecount.py` 核段数。
#  ⚠ ZF99 相对 ZF98 多一段：`ZF99 verify`（脚本声明 54 段 + 门结束 = 日志 55 段）。
# ============================================================

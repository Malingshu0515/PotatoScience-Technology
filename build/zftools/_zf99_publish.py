# -*- coding: utf-8 -*-
r"""_zf99_publish.py —— ZF99 出成品（作废 ZF98 那版 `ffa01872…`）+ 把 §9 的占位填实

照老规矩：**先核对旧哈希、再核对新产物内容，全过了才动文件**；
§4.59 ④ 的两条硬化照旧（重命名用独立的 `PREV_ROUND_SHA`；占位补丁幂等）。
本轮额外核：成品里 zh_cn 仍是 303 键、crafting_shaped 仍 = 38、改前那 44 份配方一件不少、
★ 本轮的关键自证 —— **成品里的空气分离器 class 里必须带着这次新加的粒子常量名**
（javac 编译产物里常量名会被内联掉，所以改核 class 文件里有没有
`ParticleTypes`/`sendParticles` 的引用；最稳的是核"盘上源码有、且 jar 是这次构建的"：
用 jar 的条目数与 zh_cn 内容 + 一个"泵轮次"标志位一起核）。
"""
import hashlib
import io
import json
...
SHA = DST + ".sha1"
PUB = os.path.join(ROOT, "build", "zftools", "_zf99_publish.py")
DOC = os.path.join(ROOT, "docs", u"开发档案.md")
RDIR = os.path.join(ROOT, r"src\main\resources\data\potato_s_t\recipe")
BEFORE = os.path.join(r"C:\PotatoST救援\zf99_pre", "recipe_before.txt")
VOID = "533749f3053558f8f201fc397f1c72725f80a40d"
PREV_ROUND_SHA = "ffa01872655c403c7eb5f88563eebd3c957187ee"
fails = []
...
def main():
    ...
    pump = read(os.path.join(ROOT, r"src\main\java\com\potatost\mod",
                             "AirSeparatorBlockEntity.java"))
    if u"ParticleTypes.CLOUD" not in pump or u"server.sendParticles(" not in pump:
        fails.append(u"盘上的空气分离器源码里没有粒子代码 ⇒ 是不是改错文件了？")
    else:
        print(u"  [OK]   盘上源码里确实有粒子代码（ParticleTypes.CLOUD + server.sendParticles）")
    ...
    with zipfile.ZipFile(SRC) as zf:
        ...
        rel = u"com/potatost/mod/AirSeparatorBlockEntity.class"
        if rel not in names:
            fails.append(u"成品里没有空气分离器的 class")
        else:
            raw = zf.read(rel)
            # ★ 本轮的关键自证：class 常量池里必须出现这次新加的粒子常量名
            #   （字符串常量与字段名都会进常量池；`PARTICLE_INTERVAL` 是编译期常量会被内联，
            #    所以核的是**方法名** `spawnWorkParticles` —— 它一定在）
            if b"spawnWorkParticles" not in raw:
                fails.append(u"成品里的空气分离器 class 还是旧的（找不到 spawnWorkParticles）"
                             u"⇒ 忘了重新构建？")
            else:
                print(u"  [OK]   成品里的空气分离器 class 带着这次新加的方法 "
                      u"spawnWorkParticles（%d B）" % zf.getinfo(rel).file_size)
        inside = json.loads(zf.read(u"assets/potato_s_t/lang/zh_cn.json").decode("utf-8"))
        if len(inside) != 303:
            fails.append(u"成品里 zh_cn 键数 %d ≠ 303" % len(inside))
        else:
            print(u"  [OK]   成品里 zh_cn 仍是 303 键")
        shaped = 0
        for n in names:
    ...
    print(u"   作废 %s（ZF98）" % VOID[:8])

    patch(DOC, u"__ZF99_SHA1__", new, u"§9 ZF99 条目：成品哈希填实", optional=True)
    patch(DOC, u"__ZF99_BYTES__", str(size), u"§9 ZF99 条目：字节数填实", optional=True)
    patch(DOC, u"__ZF99_ENTRIES__", str(entries), u"§9 ZF99 条目：条目数填实", optional=True)
    patch(DOC, u"**成品**：`release\\PotatoST-0.11.jar` = `%s`" % PREV_ROUND_SHA,
          u"**当时的成品**：`release\\PotatoST-0.11.jar` = `%s`" % PREV_ROUND_SHA,
          u"§9 ZF98 条目：成品 → 当时的成品", optional=True)

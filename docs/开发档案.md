...
> 这份文档是跨会话的唯一记忆。项目**不是 git 仓库**，磁盘上也没有别的档案文件——
...
- [x] `_zf107_verify.py`：**553 项，0 失败**（树形闭合 / 27 份 JSON / 判据与图标在盘上注册 /
...
- [x] `_zf107_falsify.py`：**12 把刀 K80~K91**，每把都是「改一处数据 ⇒ 校验器必须 FAIL ⇒ 逐字还原 ⇒ 回到全绿」

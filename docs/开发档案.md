...
- **每轮流程**：开工 `git status` → 收工 `git add -A` → `git commit -m "ZFnnn: 一句话"` → `git push`。
  ...
  删完一起提交。
  ⚠ **ZF107 这条规矩没执行**（2026-09-25 收尾时查证）：`build\zftools\check\` 里**没有** `Zf107Check.java`
  （该目录最新文件止于 2026-09-24），`src` 里也已经删了 ⇒ 探针**源代码已丢**，
  盘上只剩编译产物 `build\classes\java\main\com\potatost\mod\Zf107Check.class`（被 `.gitignore` 挡住，不进仓）。
  325 项探针结论本身还在：`build\zftools\_zf107_probe_utf8.txt` 和 §9 的 ZF107 条目。
  ⇒ **下次谁再写探针：`src` 删之前先 `Copy-Item`，别指望"下次补抄"**（同类失误已第二次，见 §4.x 探针那条）。
